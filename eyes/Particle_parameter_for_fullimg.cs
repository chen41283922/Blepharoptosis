...
namespace SomeCalibrations
{
    class Particle_parameter_for_fullimg
    {
        ...
        //public Image<Gray, float> cornerMap;
        //public Image<Bgr, byte> cornerShow;
        ...
        //int rad_symm = 135;
        int alpha_symm = 1;
        int HarrisBlockSize = 5; //adjust by img size 100D : 13
        int symm_rad_range = 5;//100D : 66  //indu 20  //hospi 35
        VectorOfVectorOfPoint Contours = new VectorOfVectorOfPoint();
        public VectorOfPoint ContoursPoint;

        public Particle_parameter_for_fullimg(Image<Bgr,byte>img) {

            width = img.Width;
            height = img.Height;
            grayimg = img.Convert<Gray, byte>(); //img[1];
            //**Gradient
            //horizontal filter
            ...
            CvInvoke.Threshold(saturation, saturation, 10, 255, ThresholdType.Binary);

            saturation = saturation.Not();
            CvInvoke.FindContours(saturation, Contours, null, RetrType.External, ChainApproxMethod.ChainApproxNone);
            double maxArea = 0;
            int Inx = 0;
            if (Contours.Size > 0)
            {
            ...
            Rectangle rect = CvInvoke.BoundingRectangle(Contours[Inx]);
            saturation = grayimg.Copy();
            saturation.Draw(rect, new Gray(255), 1);
            //CvInvoke.DrawContours(saturation,Contours,Inx, new MCvScalar(255, 255, 255), 1, LineType.EightConnected, null);
            ContoursPoint = Contours[Inx];

            //saturation.Draw(new CircleF(ContoursPoint[0],1), new Gray(255), 1);
            ...
            m_CornerImage = m_CornerImage.AbsDiff(new Gray(0));
            double min = 0, max = 0;
            ...
            m_SourceImage = m_CornerImage.ConvertScale<byte>(scale, shift);
            m_CornerImage.Save("m_CornerImage.jpg");
            m_SourceImage.Save("m_SourceImage.jpg");

            ////非極大值抑制
            //Image<Gray, float> Dilate = m_CornerImage.CopyBlank();
            //Image<Gray, byte> localMax = m_SourceImage.CopyBlank();

            //Mat element = CvInvoke.GetStructuringElement(ElementShape.Ellipse, new Size(3, 3), new Point(-1, -1));
            //CvInvoke.Dilate(m_CornerImage, Dilate, element, new Point(-1, -1), 1, BorderType.Constant, new MCvScalar(255, 0, 0));
            ////Dilate = m_CornerImage.Dilate(1);


            //CvInvoke.Compare(m_CornerImage, Dilate, localMax, CmpType.Equal);

            //cornerMap = m_CornerImage.CopyBlank();
            //double qualityLevel = 0.001;
            //double th = qualityLevel * max; // 閾值計算 
            //CvInvoke.Threshold(m_CornerImage, cornerMap, th, 255, ThresholdType.Binary);

            //m_SourceImage = cornerMap.Convert<Gray, byte>();

            //CvInvoke.BitwiseAnd(m_SourceImage, localMax, m_SourceImage);
            ...
            if (cornimg == null) {
                ...
                for (int i = 0; i < m_SourceImage.Height; i++)
                {
                    for (int j = 0; j < m_SourceImage.Width; j++)
                    {
                        ...
                        {
                            cornimg.Draw(new CircleF(new PointF(j, i), 0), new Bgr(Color.Red), 1);
                            
                        }
                        else if (m_SourceImage[i, j].Intensity > 100)    //通过阈值判断
                            cornimg.Draw(new CircleF(new PointF(j, i), 0), new Bgr(Color.Blue), 1);
                        else if (m_SourceImage[i, j].Intensity > 50)    //通过阈值判断
                            cornimg.Draw(new Cross2DF(new PointF(j, i), 1,1), new Bgr(Color.Green), 1);
